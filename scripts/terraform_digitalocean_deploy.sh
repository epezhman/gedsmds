...
cp "${PROJECT_ABSOLUTE_PATH}"/terraform-config/"${TFVARS}" "${PROJECT_ABSOLUTE_PATH}"/deployment/terraform-digitalocean/terraform.tfvars
...
echo Terrraform Digitanocean done in $(expr $end - $start) seconds.
